...
import {
  FaBars, FaChevronLeft, FaChevronRight, FaSun, FaMoon, FaBell, FaCog, FaUserCircle, FaLock,
  FaSignOutAlt, FaGlobe, FaAngleDown, FaPlus
} from 'react-icons/fa';

// بيانات وهمية للإشعارات (استبدلها ببيانات حقيقية من API)
const mockNotifications = [
    { id: 1, titleKey: "notification_update_title", messageKey: "notification_update_message", created_at: "2024-05-20 10:00", data: {version: "1.2"} },
    { id: 2, titleKey: "notification_leave_request_title", messageKey: "notification_leave_request_message", created_at: "2024-05-20 09:30", data: {employeeName: "أحمد"} },
];
    ...
    >
      <div className="flex items-center gap-2 sm:gap-3">
        <button
          ...
          title={isMainSidebarOpen ? t('navbar.collapseSidebar') : t('navbar.expandSidebar')}
        >
          {isMainSidebarOpen ?
            ...
            <div className={`absolute mt-2 w-72 sm:w-80 max-h-96 overflow-y-auto bg-white dark:bg-slate-800 rounded-xl shadow-xl z-50 border dark:border-slate-200 dark:border-slate-700 ${dir === 'rtl' ? 'left-0' : 'right-0'}`}>
              ...
              {mockNotifications.length > 0 ? (
                mockNotifications.map((notification) => (
                  ...
                  >
                    <div className="font-medium text-sm text-slate-700 dark:text-slate-200">{t(notification.titleKey, notification.data)}</div>
                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{t(notification.messageKey, notification.data)}</div>
              ...
              <div className="p-2 text-center border-t dark:border-slate-700 sticky bottom-0 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm">
                <Link to="/notifications" onClick={() => setIsNotificationsOpen(false)} className="text-sm text-sky-600 dark:text-sky-400 hover:underline">
        ...
        <div className="relative" ref={settingsRef}>
          ...
          >
            {user && user.profileImageUrl ? (
              <img src={user.profileImageUrl} alt={user.name || t('navbar.user_avatar_alt')} className="w-full h-full object-cover" />
            ) : (
          ...
          {isSettingsOpen && (
            <div className={`absolute mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-xl z-50 border dark:border-slate-200 dark:border-slate-700 overflow-hidden ${dir === 'rtl' ? 'left-0' : 'right-0'}`}>
              <div className="px-4 py-3 border-b dark:border-slate-700">
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 truncate">{user?.name || t('navbar.guest')}</p>
              </div>
              <NavLink // <-- استخدام NavLink هنا
                to="/profile"
                className={({ isActive }) => `flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-slate-100 dark:hover:bg-slate-700/50 ${isActive ? 'text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-700/30' : 'text-slate-700 dark:text-slate-300'}`}
                onClick={() => setIsSettingsOpen(false)}
              >
                <FaUserCircle className="text-slate-500 dark:text-slate-400" /><span>{t('navbar.profile')}</span>
              </NavLink>
              <NavLink // <-- استخدام NavLink هنا
                ...
                onClick={() => setIsSettingsOpen(false)}
              >
                <FaLock className="text-slate-500 dark:text-slate-400" /><span>{t('navbar.changePassword')}</span>
              </NavLink>
              <button
          ...
          {isLanguageDropdownOpen && (
            <div className={`absolute mt-2 w-36 bg-white dark:bg-slate-800 rounded-md shadow-lg border dark:border-slate-200 dark:border-slate-700 z-50 overflow-hidden ${dir === 'rtl' ? 'left-0' : 'right-0'}`}>
              <ul className="py-1">
                <li>
                  <button
                    onClick={() => { switchLanguageHandler('ar'); setIsLanguageDropdownOpen(false); }}
                ...
                <li>
                  <button
                    onClick={() => { switchLanguageHandler('en'); setIsLanguageDropdownOpen(false); }}
