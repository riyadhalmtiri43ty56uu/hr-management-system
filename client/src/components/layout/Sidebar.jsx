...
const SidebarItem = React.memo(
  ({
    ...
    const filteredSubItems = React.useMemo(() => {
      return (
        item.subItems?.filter((sub) => !sub.condition || sub.condition(user)) ||
        []
      );
    }, [item.subItems, user]);
    ...
    const calculateIsActive = useCallback(() => {
      ...
      return filteredSubItems.some((subItem) => {
        ...
        const subItemPathWithSlash = subItem.path.endsWith("/")
          ? subItem.path
          : subItem.path + "/";
        const currentPathWithSlash = location.pathname.endsWith("/")
          ? location.pathname
          : location.pathname + "/";
        ...
        return (
          currentPathWithSlash.startsWith(subItemPathWithSlash) ||
    ...
    return (
      ...
      <div className="mb-1">
        ...
        >
          ...
          {!(isOpen || isMobileView) && (
            <span
              className={`absolute z-20 hidden group-hover:block bg-slate-700 dark:bg-slate-900 text-white text-xs px-2 py-1 rounded shadow-md whitespace-nowrap
          ...
          {(isOpen || isMobileView) &&
            ...
            (dir === "rtl" ? (
              <FaAngleLeft
                size={14}
                className={`text-slate-400 group-hover:text-sky-500 transition-transform duration-200 ${
        ...
        {(isOpen || isMobileView) && hasSubItems && isSubMenuOpen && (
          <div
            className={`py-1 space-y-0.5 overflow-hidden transition-all duration-300 ease-in-out ${
              dir === "rtl"
                ? "pr-6 mr-1 border-r-2 border-sky-500/20"
                : "pl-6 ml-1 border-l-2 border-sky-500/20"
            }`}
          >
...
const Sidebar = ({ isOpen, user, isMobileView = false, onLinkClick }) => {
  ...
  const toggleSubMenu = useCallback((menuId) => {
    setActiveSubMenu((prevActiveSubMenu) =>
      prevActiveSubMenu === menuId ? null : menuId
    );
  }, []);
  ...
  useEffect(() => {
    ...
    for (const item of menuConfig) {
      ...
      if (filteredSubItems.length > 0) {
        // فقط إذا كان العنصر له قائمة فرعية مؤهلة
        const isActiveParent = filteredSubItems.some((subItem) => {
          ...
          const subItemPathWithSlash = subItem.path.endsWith("/")
            ? subItem.path
            : subItem.path + "/";
          const currentPathWithSlash = currentPath.endsWith("/")
            ? currentPath
            : currentPath + "/";
  ...
  return (
    ...
    >
      <div
        className={`flex items-center border-b border-slate-200/80 dark:border-slate-700/80
                      ${
                        isMobileView
                          ? "p-4 justify-between h-16"
                          : isOpen
                          ? "px-4 justify-start h-[88px]"
                          : "py-4 justify-center h-[64px]"
                      }`}
      >
        {isMobileView ? (
          <>
          ...
          <Link
            to={
              menuConfig.find((item) => item.id === "dashboard_main")?.path ||
              "/dashboard"
            }
            className="block group w-full flex justify-center items-center"
          >
            <img
              src={
                isOpen
                  ? user?.logoUrl || "/assets/images/IMG_0294.JPG"
                  : user?.companyLogoShort ||
                    "/assets/images/logo-icon-placeholder.png"
              }
              alt={t("sidebar.companyLogoAlt")}
              className={`${
                isOpen ? "h-10 max-w-[160px]" : "h-9 w-9"
      ...
      <nav
        className="flex-1 px-2.5 py-3 space-y-1 overflow-y-auto custom-scrollbar custom-scrollbar-firefox 
      ...
      {!isMobileView && (
        <div className="p-3 border-t border-slate-200/80 dark:border-slate-700/80 mt-auto">
          {isOpen ? (
            <Link
              to="/profile"
              className={`flex items-center rounded-md p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700/40 ${
                dir === "rtl"
